...
class UserQueryProcessor(
    ...
    fun validateUserIdentifier(
        email: String,
        phoneNumber: String?,
    ) {
        if (existsByEmail(email)) {
            throw IllegalArgumentException("User identifier already exists: $email")
        }

        if (phoneNumber != null && existsByPhoneNumber(phoneNumber)) {
            throw IllegalArgumentException("User identifier already exists: $phoneNumber")
        }
    }
}
