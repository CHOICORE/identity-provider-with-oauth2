...
class UserRepositoryTest {
    ...
    UserRepositoryTest(final UserRepository userRepository, final AuthorityRepository authorityRepository, final GrantedAuthorityRepository grantedAuthorityRepository, final TestEntityManager testEntityManager) {
    ...
    void t1() {
        ...
        User registerUser = User.builder()
                .credential(credential).username(username).nickname(nickname).picture(profileImageUri).gender(gender).accountStatus(accountStatus).build();

        userRepository.save(registerUser);

        authorityRepository.findByScope("ACCOUNT").stream().filter(authority -> userAuthorityTypes.contains(UserAuthorityType.valueOf(authority.getName()))).map(authority -> GrantedAuthority.builder().userId(registerUser.getId()).authority(authority).grantedAt(Instant.now()).build()).forEach(grantedAuthorityRepository::save);
